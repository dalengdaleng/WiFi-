...
- (BOOL)shouldCancleHandleBodyData
{
    return NO;
}
...
/**
 * Called if we're busy.
 **/
- (void)handleServiceUnavailable
{
	HTTPLogInfo(@"HTTP Server: Error 503 - server busy (%@)", [self requestURI]);
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	if (tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
            ...
            int code;
			BOOL expectsUpload = [self expectsRequestBodyFromMethod:method atPath:uri statusCode:&code];
	...
	else
	{
		...
		else if (tag == HTTP_REQUEST_CHUNK_DATA)
		{
			...
			if ([self shouldCancleHandleBodyData])
            {
                [self handleServiceUnavailable];
                return;
            }
